...
import { useRoqPlatformUserProfiles, useBookingCount, useDrivingLicenseCount, useMaintenanceCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const drivingLicenseStat = useDrivingLicenseCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: drivingLicenseStat?.data,
      isLoading: drivingLicenseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Maintenances',
